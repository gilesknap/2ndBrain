...
### 1.2 GPG + pass Infrastructure

**Risk Level: Medium**

The rclone config encryption password is stored in `pass` (GPG-encrypted).
The `setup-gpg-pass.sh` script configures extremely long cache TTLs
(`default-cache-ttl 34560000` ≈ 400 days) and uses `allow-preset-passphrase`
so the agent never re-prompts.
...
- The passphrase is never written to disk in plaintext (cached in
  `gpg-agent` memory only)
- `keygrip.txt` is `chmod 600`

**Residual risk:**
- The 400-day TTL means a compromised session has prolonged access to the
  decrypted material — the passphrase effectively never expires while the
  machine is running
...
**Existing mitigations:**
- rclone config is encrypted with GPG via `pass`
...
| GPG cache TTL                   | Medium     | 400-day passphrase cache              |
...
10. **Reduce GPG cache TTL.**
    Lower `default-cache-ttl` and `max-cache-ttl` to a more reasonable
    value (e.g. 86400 = 24 hours) in `setup-gpg-pass.sh`. The preset
    mechanism can re-cache the passphrase on service restart.

11. **Truncate or redact attachment filenames in logs.**
...
12. **Add a `--dry-run` mode to `VaultEditAgent`.**
...
13. **Consider using Slack's verified request signing.**
...
14. **Audit oEmbed response handling.**
